        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          ...
          <div className="glass-card p-5 sm:p-6 relative overflow-hidden group">
            ...
            <p className="text-2xl sm:text-3xl font-extrabold text-amber-400 font-score">
              {loading ? '...' : (stats.fundBalance || 5200000).toLocaleString('vi-VN')} <span className="text-xs text-slate-400">VNĐ</span>
            </p>
            <p className="text-xs text-slate-400 mt-1.5">
      ...
      {activeTournament && (
        <section className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="relative rounded-3xl overflow-hidden border border-orange-500/30 bg-gradient-to-br from-navy-900/90 via-navy-850/80 to-navy-900/90 backdrop-blur-2xl p-6 sm:p-8 lg:p-10 shadow-2xl shadow-orange-950/30">
            ...
            {/* Quick Live Matches / Standings Teaser */}
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 pt-6">
              <div className="glass-card p-4 rounded-2xl space-y-3">
                <div className="flex items-center justify-between text-xs text-slate-400 font-semibold uppercase tracking-wider">
                  <span className="flex items-center gap-1.5 text-orange-400">
                    <Activity className="w-3.5 h-3.5" /> Sân 1 (Đang Đấu)
                  </span>
                  <span className="badge-emerald">Game 1</span>
                </div>
                <div className="space-y-2">
                  <div className="flex justify-between items-center bg-white/[0.03] p-2.5 rounded-xl border border-white/[0.05]">
                    <span className="text-sm font-bold text-white">Hùng K98 / Mai Tennis</span>
                    <span className="text-xl font-extrabold text-orange-400 font-score">09</span>
                  </div>
                  <div className="flex justify-between items-center bg-white/[0.03] p-2.5 rounded-xl border border-white/[0.05]">
                    <span className="text-sm font-bold text-white">Tuấn Đập / Lan Tốc Độ</span>
                    <span className="text-xl font-extrabold text-slate-400 font-score">07</span>
                  </div>
                </div>
              </div>
              ...
              <div className="glass-card p-4 rounded-2xl space-y-3">
                <div className="flex items-center justify-between text-xs text-slate-400 font-semibold uppercase tracking-wider">
                  <span className="flex items-center gap-1.5 text-cyan-400">
                    <Calendar className="w-3.5 h-3.5" /> Sân 2 (Tiếp Theo)
                  </span>
                  <span className="badge-blue">Sắp đấu</span>
                </div>
                <div className="space-y-2">
                  <div className="flex justify-between items-center bg-white/[0.03] p-2.5 rounded-xl border border-white/[0.05]">
                    <span className="text-sm font-bold text-white">Đức Xoáy / Hoa Bền</span>
                    <span className="text-xs text-slate-500 font-semibold">—</span>
                  </div>
                  <div className="flex justify-between items-center bg-white/[0.03] p-2.5 rounded-xl border border-white/[0.05]">
                    <span className="text-sm font-bold text-white">Cường Lưới / Dung Smash</span>
                    <span className="text-xs text-slate-500 font-semibold">—</span>
                  </div>
                </div>
              </div>
                ...
                <div>
                  <div className="flex items-center justify-between text-xs text-slate-400 font-semibold uppercase tracking-wider mb-2">
                    <span className="flex items-center gap-1.5 text-amber-400">
                      <Trophy className="w-3.5 h-3.5" /> Bảng Xếp Hạng Nhanh
                    </span>
                    <Link to={`/tournaments/${activeTournament.id}/standings`} className="text-orange-400 hover:underline">
                      Xem hết
                    </Link>
                  </div>
                  <p className="text-xs text-slate-300 leading-relaxed">
                    Theo dõi điểm số, hiệu số phụ và nhánh đấu trực tiếp cập nhật tức thì khi trọng tài xác nhận kết quả.
                  </p>
                </div>
                <Link
                  to={`/tournaments/${activeTournament.id}/standings`}
                  className="mt-3 inline-flex items-center justify-center gap-1.5 text-xs font-bold text-orange-400 hover:text-orange-300 py-2 rounded-xl bg-orange-500/10 border border-orange-500/20"
                >
                  Xem Toàn Bộ Bảng Điểm <ChevronRight className="w-3.5 h-3.5" />
                </Link>
              </div>
            </div>
          </div>
